...
http-request ^https?://(zeeho\.box|.*zeehoev\.com)/.* script-path=https://raw.githubusercontent.com/mlink798/ZEEHO/refs/heads/main/repo/zeeho_box_enhanced.js, requires-body=true, timeout=60, tag=极核面板V2.14.1
...
// ============================================
const SCRIPT_VERSION = "v2.14.1";
console.log(`🚀 [极核面板] 脚本版本: ${SCRIPT_VERSION} (2026-09-25 v2.14.1 修复手机号登录“验证码有误或已过期”：发码与登录统一改走App网关，与HA集成同验证码池)`);
...
!(async () => {
  ...
  if (method === "POST" && path === "/api/phone-login") {
    ...
    const loginRes = await httpPost(
      loginUrl,
      { "Content-Type": "application/json;charset=UTF-8", "User-Agent": "okhttp/4.9.2", ...signH },
      payload,
      20000
    );
